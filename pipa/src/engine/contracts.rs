//! Contract management functions for the engine
...
pub fn get_contract(name: &str) -> (ContractInfo, String) {
    ...
    if !Path::new(&contract_path).exists() {
        let message = log_action("contract_retrieved", Some("exists=false"), Some(name), None, None);
        return (ContractInfo {
            name: name.to_string(),
            version: "".to_string(),
            exists: false,
    ...
    let message = log_action("contract_retrieved", Some("exists=true"), Some(&contract.contract.name), Some(&contract.contract.version), None);
    (ContractInfo {
